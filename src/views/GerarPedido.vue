  ...
  methods: {
    ...
    buscaClientes () {
      ...
      if (this.clientes === null) {
        ...
        axios.get('http://localhost:8081/clientes?token=' + token)
          .then((response) => {
    ...
    buscaTransportadoras () {
      ...
      if (this.transportadoras === null) {
        ...
        const token = sessionStorage.getItem('token')
        axios.get('http://localhost:8081/transportadoras?token=' + token)
          .then((response) => {
            this.checkInvalidLoginResponse(response.data)
    ...
    buscaPedidosCliente () {
      if (this.cliente !== '') {
        ...
        const token = sessionStorage.getItem('token')
        axios.get('http://localhost:8081/pedidosCliente?cli=' + this.cliente + '&token=' + token)
          .then((response) => {
            this.checkInvalidLoginResponse(response.data)
    ...
    buscaPedidos () {
      ...
      const token = sessionStorage.getItem('token')
      axios.get('http://localhost:8081/pedidos?token=' + token)
        .then((response) => {
          this.checkInvalidLoginResponse(response.data)
    ...
    buscaCondicoesPagto () {
      if (this.empresa !== '') {
        ...
        const token = sessionStorage.getItem('token')
        axios.get('http://localhost:8081/condicoesPagto?emp=' + this.empresa + '&token=' + token)
          .then((response) => {
            this.checkInvalidLoginResponse(response.data)
    ...
    buscaEstilos (item) {
      ...
      const token = sessionStorage.getItem('token')
      axios.get('http://localhost:8081/estilos?emp=1&token=' + token)
        .then((response) => {
          this.checkInvalidLoginResponse(response.data)
    ...
    buscaConfigs (item, estilo) {
      if (estilo) {
        ...
        document.getElementById('btnBuscaComps' + item.hash).disabled = true
        const token = sessionStorage.getItem('token')
        axios.get('http://localhost:8081/produtosPorEstilo?emp=1&estilo=' + estilo + '&token=' + token)
          .then((response) => {
            this.checkInvalidLoginResponse(response.data)
    ...
    buscaComps (item, config) {
      if (config) {
        ...
        document.getElementById('btnBuscaComps' + item.hash).disabled = true
        const token = sessionStorage.getItem('token')
        axios.get('http://localhost:8081/derivacoesPorProduto?emp=1&produto=' + config + '&token=' + token)
          .then((response) => {
            this.checkInvalidLoginResponse(response.data)
    ...
    buscarDadosCliente (codCli, apenasEmpresas) {
      const token = sessionStorage.getItem('token')
      axios.get('http://localhost:8081/dadosCliente?token=' + token + '&codCli=' + codCli)
        .then((response) => {
          this.checkInvalidLoginResponse(response.data)
    ...
    enviarItens () {
      ...
      if (!temErro) {
        ...
        const headers = { headers: { 'Content-Type': 'application/json' } }
        axios.post('http://localhost:8081/pedido/itens?token=' + token, body, headers)
          .then((response) => {
            this.checkInvalidLoginResponse(response.data)
            parseString(response.data, { explicitArray: false }, (err, result) => {
              if (err) {
    ...
    buscarDerivacoes (config) {
      const token = sessionStorage.getItem('token')
      return axios.get('http://localhost:8081/derivacoesPorProduto?emp=1&produto=' + config + '&token=' + token)
    },
    compare (a, b) {
      if (a.cnj < b.cnj) {
        return -1
    ...
    carregarCabecalho () {
      const token = sessionStorage.getItem('token')
      axios.get('http://localhost:8081/pedido?emp=' + this.empresa + '&fil=1&ped=' + this.numPed + '&token=' + token)
        .then((response) => {
    ...
    enviarEmpresa () {
      ...
      if (!this.itens.length) {
        ...
        document.getElementsByTagName('body')[0].style.cursor = 'wait'
        const token = sessionStorage.getItem('token')
        axios.post('http://localhost:8081/enviarPedido?emp=' + this.empresa + '&fil=1&ped=' + this.numPed + '&token=' + token)
          .then((response) => {
            this.checkInvalidLoginResponse(response.data)
    ...
    onUploadArquivo (item) {
      ...
      const token = sessionStorage.getItem('token')
      axios.post('http://localhost:8081/uploadArquivo?emp=' + this.empresa + '&fil=1&ped=' + this.numPed + '&ipd=' + item.seqIpd + '&token=' + token, this.formData, { headers: headers })
        .then((response) => {
          if (response.data === 'OK') {
            item.temAnx = 'S'
    ...
    download (item) {
      document.getElementsByTagName('body')[0].style.cursor = 'wait'
      const token = sessionStorage.getItem('token')
      axios.get('http://localhost:8081/downloadArquivo?emp=' + this.empresa + '&fil=1&ped=' + this.numPed + '&ipd=' + item.seqIpd + '&token=' + token)
      axios({
        url: 'http://localhost:8081/downloadArquivo?emp=' + this.empresa + '&fil=1&ped=' + this.numPed + '&ipd=' + item.seqIpd + '&token=' + token, // your url
        method: 'GET',
        responseType: 'blob'
      })
        .then((response) => {
