...
export default {
  ...
  methods: {
    ...
    async buscarOpcoes (item) {
      ...
      if (item.proGen === 'S') {
        await axios.get('http://localhost:8081/equivalentes?emp=' + this.codEmp + '&modelo=' + item.codMod + '&componente=' + item.codPro + '&derivacao=' + item.codDer + '&token=' + token)
          .then((response) => {
        ...
        await axios.get('http://localhost:8081/equivalentes?emp=' + this.codEmp + '&modelo=' + item.codMod + '&componente=' + item.codPro + '&derivacao=' + item.codDer + '&token=' + token)
          .then((response) => {
            ...
            if (!item.equivalentes.length) {
              item.equivalentes = []
              item.equivalentesFiltrados = []
              axios.get('http://localhost:8081/derivacoesPossiveis?emp=' + this.codEmp + '&pro=' + item.codPro + '&mod=' + item.codMod + '&derMod=' + item.derMod + '&token=' + token)
                .then((response) => {
                  this.checkInvalidLoginResponse(response.data)
        ...
        await axios.get('http://localhost:8081/equivalentes?emp=' + this.codEmp + '&modelo=' + item.codMod + '&componente=' + item.codPro + '&derivacao=' + item.codDer + '&token=' + token)
          .then(async (response) => {
            ...
            if (!item.equivalentes.length) {
              item.equivalentes = []
              item.equivalentesFiltrados = []
              await axios.get('http://localhost:8081/derivacoesPossiveis?emp=' + this.codEmp + '&pro=' + item.codPro + '&mod=' + item.codMod + '&derMod=' + item.derMod + '&token=' + token)
                .then((response) => {
                  this.checkInvalidLoginResponse(response.data)
                  item.equivalentes = response.data.derivacoes
                  item.equivalentesFiltrados = response.data.derivacoes
    ...
    confirmarTroca (equivalente) {
      ...
      const token = sessionStorage.getItem('token')
      axios.get('http://localhost:8081/itensMontagem?emp=' + this.codEmp + '&pro=' + itemTroca.cmpAtu + '&der=' + itemTroca.derAtu + '&token=' + token)
        .then((response) => {
          this.checkInvalidLoginResponse(response.data)
